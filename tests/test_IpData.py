...
class DummyLogLine:
    def __init__(self, path, http_status, req_ts, upstream_response_time):
        self.path = path
...
def test_min_max_ts():
    ...
    ipd = IpData("1.2.3.4", "ip", {"log_lines": log_lines})
...
def test_total_and_avg_time():
    ...
    for ll in make_log_lines():
        log_lines.append(None, ll)
    ipd = IpData("1.2.3.4", "ip", {"log_lines": log_lines})
    assert ipd.total_time == pytest.approx(3.8)
    assert ipd.avg_time == pytest.approx(0.76)
...
def test_steal_time_and_steal_ratio():
    ...
    for ll in make_log_lines():
        log_lines.append(None, ll)
    ipd = IpData("1.2.3.4", "ip", {"log_lines": log_lines})
    assert ipd.steal_time == pytest.approx(4 - 3.8)
    assert ipd.steal_ratio == pytest.approx((3.8 / 4) * 100.0)