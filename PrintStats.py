...
class PrintStats:
    ...
    def print_stats(
        ...
        print(
            f"Running: {time.time() - config['start_time']:.2f} seconds  |  Parsed: {config['lines_parsed']} lines  |  Time frame: {config['time_frame']} seconds"
        )
        print(
        ...
        for ip, stats in sorted(
            ip_stats.items(),
            key=lambda item: (item[1].http_status_bad),
            reverse=True,
        ):
            counter += 1
            ...
            status_stats = (
                f"{stats.http_status_bad} ({stats.http_status_bad_perc:.2f}%)"
            )
#            referer_stats = (
#                f"{stats.referer['unrelated']:6d} {stats.referer['no_referer']:6d} "
#            )
            referer_stats = ""
            message = (
...
#                f"{stats.score:>{PrintStats.column_6_width}.2f}|"
                f"{stats.request_count:>{PrintStats.column_6_width}.2f}|"
            )
            if ip in banned_ips:
...
#            elif stats.score >= 1:
        ...
        if False:
            for ip, data in sorted(
                ip_stats.items(),
                key=lambda item: (
                    item[1]["http_status"]["bad_perc"],
                    item[1]["http_status"]["count"],
                ),
                reverse=True,
            ):
                if counter > config["detail_lines"]:
        ...
        if False and config["referer_stats"]:
            ...
            for ip, stats in sorted(
                referer_stats.items(),
                key=lambda item: int(item[1]["unrelated"]) + int(item[1]["no_referer"]),
                reverse=True,
            ):
                counter += 1
                if counter > config["detail_lines"]:
        ...
        if False and config["url_stats"]:
            print()
            counter = 0
            for url, stats in sorted(
                url_stats.items(),
                key=lambda item: (item[1]["request_count"], item[1]["total_time"]),
                reverse=True,
            ):
                counter += 1
                if counter > config["detail_lines"]:
        ...
        if False and config["ua_stats"]:
            print()
            counter = 0
            for ua, stats in sorted(
                ua_stats.items(), key=lambda item: item[1]["total_time"], reverse=True
            ):
                # if stats['total_time'] < 1:
                #    continue
                counter += 1
        ...
        for ip in banned_ips:
            print(
                f"{ip}({config['ban_time'] - (current_time - banned_ips[ip]):.0f}s)",
                end=", ",
            )
        print()
