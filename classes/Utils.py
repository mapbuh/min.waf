...
class Utils:
    ...
    def requests_get_cached(
        ...
        if not cache_dir:
            cache_dir = Utils.cache_dir_path()
        ...
        t = threading.Thread(target=Utils.fetch_and_cache, args=(url, timeout, cache_file))
        t.start()
    ...
    def requests_get_cached_json(
        ...
        """Fetch a URL with caching to avoid repeated requests."""
        data = Utils.requests_get_cached(url, timeout, cache_dir, ttl, strict, logger)
